...
package utils.pekkohttpcirce

import org.apache.pekko.http.javadsl.common.JsonEntityStreamingSupport
import org.apache.pekko.http.scaladsl.common.EntityStreamingSupport
import org.apache.pekko.http.scaladsl.marshalling.{Marshaller, Marshalling, ToEntityMarshaller, ToResponseMarshaller}
import org.apache.pekko.http.scaladsl.model.{
  ContentType, ContentTypeRange, HttpEntity, HttpResponse, MediaType, MediaTypes, MessageEntity, StatusCode, StatusCodes
}
...
//import wsActor.WsActor.TypeAndContentOut
import scala.collection.immutable.Seq
import scala.concurrent.Future
import scala.util.control.NonFatal
import scala.util.Try

/** Automatic to and from JSON marshalling/unmarshalling using an in-scope circe protocol. The unmarshaller fails fast,
  * throwing the first `Error` encountered.
  *
...
/** Automatic to and from JSON marshalling/unmarshalling using an in-scope circe protocol. The unmarshaller accumulates
  * all errors in the exception `Errors`.
  *
  * To use automatic codec derivation, user needs to import `io.circe.generic.auto._`.
  */
...
trait BaseCirceSupport {
  ...
  implicit def tupleMarshaller[A: Encoder]: ToResponseMarshaller[(StatusCode, A)] =
    Marshaller.withFixedContentType(MediaTypes.`application/json`) {
      case (statusCode, data) if statusCode.allowsEntity() =>
        HttpResponse(
          status = statusCode,
          entity = HttpEntity(MediaTypes.`application/json`, data.asJson.noSpaces)
        )
      case (statusCode, _)                                 =>
        HttpResponse(status = statusCode)
    }

  implicit val statusCodeEncoder: Encoder[StatusCode] = Encoder.instance { statusCode =>
    Json.obj(
      "code"    -> Json.fromInt(statusCode.intValue()),
      "message" -> Json.fromString(statusCode.defaultMessage())
    )
  }
}
...
trait FailFastUnmarshaller { this: BaseCirceSupport =>
  implicit final override def fromByteStringUnmarshaller[A: Decoder]: Unmarshaller[ByteString, A] =
    byteStringJsonUnmarshaller.map(Decoder[A].decodeJson).map(_.fold(throw _, identity))
